<script>
  ...
  import { ignoreNews } from '../../news.js';
  ...
  export function setContent(newId, newSmallTitle, newLargeTitle, newLearnMoreUrl) {
    id = newId;
  ...
  export function hideItem(callback) {
    if (!shown) {
  ...
  function hideSelf() {
    self.style.transform = "translateY(150%)";
  }

  onMount(hideSelf)
</script>

<div bind:this={self} class="{!shown ? "hidden" : undefined} bg-indigo-600 fixed bottom-0 right-0 w-full transition duration-250 ease-in-out">
